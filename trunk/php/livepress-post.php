...
class livepress_post {
     ...
     */
    private static function hasChild($node) {
        ...
        for ($number = 0; $number < $all_node->childNodes->length; $number++) {
            ...
            if ($is_chunk_element) {
                ...
                if (in_array(self::$stub_chunks_class, $top_node_classes)) {
                    if (self::hasChild($top_node) || strlen(trim($top_node->nodeValue))) {
                        $nodes = array();
     ...
     */
    private function enclose_indexes_by_chunk_mark_tag($nodes_indexes,
            $parent_node, $dom, $current_ids) {
        $nodes = array();
        ...
        return $this->enclose_nodes_by_chunk_mark_tag($nodes, $parent_node,
                $dom, $current_ids, $ref_node);
    }
    ...
    /**
     * Converts a string to a DOMElement
     *
     * @param   string      $to_be_parsed   The string to be parsed into a DOMElement
     * @return  DOMElement  The DOMElement generated from the string $to_be_parsed
     */
    private function str_to_dom($to_be_parsed) {
        $sxe = simplexml_load_string($to_be_parsed);
        if ($sxe === false) {
            throw new Exception('Error while parsing an HTML');
        }

        $dom_sxe = dom_import_simplexml($sxe);
        if (!$dom_sxe) {
            throw new Exception('Error while converting an HTML');
        }
     ...
     *
     * @param   array  $current_ids An array with the strings of the used ids
